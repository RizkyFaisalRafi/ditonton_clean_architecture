...
import 'see_more_popular_movie_bloc_test.mocks.dart';
...
void main() {
  ...
  test('initial state should be Initial', () {
    expect(
      ...
      const SeeMorePopularMovieState.initial(),
    );
  });
  ...
  group('FetchInitialPopularMovies', () {
    test(
      ...
      () async {
        ...
        final expected = [
          const SeeMorePopularMovieState.loading(),
    ...
    test(
      ...
      () async {
        ...
        final expected = [
          const SeeMorePopularMovieState.loading(),
          const SeeMorePopularMovieState.error('Server Failure'),
        ];
        expectLater(seeMorePopularMovieBloc.stream, emitsInOrder(expected));
  ...
  group('FetchMorePopularMovies', () {
    test(
      ...
      () async {
        ...
        seeMorePopularMovieBloc.add(
          const SeeMorePopularMovieEvent.fetchMorePopularMovies(),
        );
      },
    );
    ...
    test('should not call usecase when hasMorePopular is false', () async {
      ...
      await seeMorePopularMovieBloc.stream.firstWhere(
        (state) => (state as Loaded).hasMorePopular == false,
