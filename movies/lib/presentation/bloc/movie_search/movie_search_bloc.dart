...
class MovieSearchBloc extends Bloc<MovieSearchEvent, MovieSearchState> {
  ...
  Future<void> _onQueryChanged(
    ...
    if (query.trim().isEmpty) {
      emit(const MovieSearchState.movieSearchEmpty());
      return;
    }
    // 2. Jika query mengandung karakter tidak valid, emit state Error.
    if (RegExp(r'[!@#$%^&*(),.?"{}|<>]').hasMatch(query)) {
      emit(
        const MovieSearchState.movieSearchError('Query contains invalid characters'),
      );
      return;
    }
    ...
    emit(const MovieSearchState.movieSearchLoading());
