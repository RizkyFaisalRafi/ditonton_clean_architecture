...
void main() {
  ...
  void arrangeBlocsState(
    ...
    OnTheAirTvState onTheAirState,
    PopularTvState popularState,
    TopRatedTvState topRatedState,
  ) {
  ...
  Widget makeTestableWidget(Widget body) {
    return MultiBlocProvider(
      ...
      child: MaterialApp(
        ...
        routes: {
          searchTvRoute: (_) => const Scaffold(body: Text('Search Tv Series')),
          onTheAirTvRoute:
  ...
  group('Tv Series Page', () {
    ...
    testWidgets('Page should display error widget when state is Error', (
      ...
      await tester.runAsync(() async {
        ...
        arrangeBlocsState(
          const AiringTodayTvState.errorAtTv(errorMessage),
          const OnTheAirTvState.errorOtaTv(errorMessage),
          const PopularTvState.errorPopularTv(errorMessage),
          const TopRatedTvState.errorTrTv(errorMessage),
        );
    ...
    testWidgets('Pull to refresh should call refreshTvSeries event', (
      ...
      verify(
        mockAiringTodayTvBloc.add(const AiringTodayTvEvent.refreshTvAt()),
      ).called(1);

      verify(mockOnTheAirTvBloc.add(OnTheAirTvEvent.refreshTvOta())).called(1);

      verify(mockPopularTvBloc.add(PopularTvEvent.refreshPTv())).called(1);

      verify(mockTopRatedTvBloc.add(TopRatedTvEvent.refreshTvTr())).called(1);
    });
  });
}
